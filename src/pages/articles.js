...
const FeaturedArticle = ({img, title, time, summary, link}) => {
    return(
        <li className='col-span-1 w-full p-4 border border-solid rounded-2xl relative
            ...
            <Link href={link} target='_blank'>
                <h2 className='capitalize font-bold mb-2 mt-4 hover:underline
                               text-lg sm:text-2xl'>
                    {title}
                </h2>
            </Link>
    ...
    return(
            <Link href={link} target='_blank' onMouseMove={handleMouse} onMouseLeave={handleMouseLeave}>
                ...
                <FramerImage ref={imgRef} src={img} alt={title}
                             ...
                             className='w-96 h-auto absolute rounded-lg z-10 hidden'/>
            </Link>
    )
}
...
const Article = ({img, title, date, link}) => {
    return(
        <motion.li className='relative w-full px-4 py-6 my-4 rounded-xl items-center justify-between bg-light dark:bg-dark
            ...
            <span className='text-primary dark:text-primaryDark font-semibold
                             self-start md:self-end md:pl-4
                             text-sm sm:text-lg'>
                {date}
            </span>
        </motion.li>
